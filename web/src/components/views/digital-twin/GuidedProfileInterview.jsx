...
export default function GuidedProfileInterview({ context, onProposal }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(err.message || "No se pudo recuperar la conversación guardada.");
      } finally {
  ...
  function refineProposal() {
    setProposal(null);
    setDraft("Quiero ajustar la propuesta: ");
  }
  ...
  return <div className="rounded-[22px] border border-indigo-400/20 bg-gradient-to-br from-indigo-500/[0.08] via-slate-900/80 to-slate-950/90 p-5 shadow-[0_16px_45px_rgba(0,0,0,0.22)]">
    ...
    {restoring ? <div className="mt-5 text-xs text-slate-500">Recuperando conversación guardada…</div> : messages.length === 0 ? <button onClick={start} disabled={loading} className="mt-5 rounded-xl bg-gradient-to-r from-indigo-500 to-blue-500 px-4 py-2.5 text-xs font-medium text-white shadow-lg shadow-indigo-950/30 disabled:opacity-50">{loading ? "Preparando..." : "Empezar conversación guiada"}</button> : <>
      ...
      {!proposal && <div className="mt-4 flex gap-2"><textarea autoFocus rows={2} value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }} placeholder="Respondé con tus palabras..." className="min-w-0 flex-1 resize-none rounded-xl border border-slate-700/70 bg-slate-950 px-3 py-2.5 text-xs leading-5 text-slate-200 outline-none placeholder:text-slate-600 focus:border-indigo-500" /><button onClick={send} disabled={!draft.trim() || loading} className="rounded-xl bg-gradient-to-r from-indigo-500 to-blue-500 px-4 text-sm text-white disabled:opacity-40">{loading ? "…" : "➤"}</button></div>}
    </>}
    {proposal && <div className="mt-5 rounded-2xl border border-emerald-400/20 bg-emerald-500/[0.05] p-4"><div className="text-[10px] uppercase tracking-[0.16em] text-emerald-300">Esto es lo que entendí de vos</div><p className="mt-3 whitespace-pre-line text-xs leading-6 text-slate-300">{proposal.investor_narrative}</p><div className="mt-4 flex flex-wrap gap-2">{[proposal.style, proposal.concentration_tolerance, proposal.drawdown_tolerance, proposal.implementation_style].filter(Boolean).map((x) => <span key={x} className="rounded-full border border-slate-700/70 bg-slate-950/60 px-2.5 py-1 text-[10px] text-slate-300">{x}</span>)}</div><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => onProposal?.(proposal)} className="rounded-xl bg-gradient-to-r from-emerald-500/90 to-cyan-500/90 px-4 py-2.5 text-xs font-medium text-slate-950">Usar esta propuesta en mi perfil</button><button onClick={refineProposal} className="rounded-xl border border-slate-600/70 bg-slate-950/50 px-4 py-2.5 text-xs font-medium text-slate-300 hover:border-indigo-400/50 hover:text-white">Quiero ajustar esta propuesta</button></div><p className="mt-2 text-[10px] leading-4 text-slate-500">Podés corregir cualquier interpretación antes de guardarla como tu perfil.</p></div>}
