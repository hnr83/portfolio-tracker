  ...
  return (
    <div className={`rounded-2xl border p-3 shadow-sm md:p-4 ${accentClass}`}>
      <div className="flex items-start justify-between gap-2 md:gap-4">
        ...
        <div
          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl md:h-10 md:w-10 ${
            positive
...
export default function MarketView({
  ...
  const { marketData, loading, error, refreshAll } = usePortfolioData();
  ...
  const filteredAndSortedMarket = useMemo(() => {
    ...
    const filtered = (marketData || []).filter((row) => {
      ...
      const matchesType =
        marketTypeFilter === "ALL" || assetType === marketTypeFilter;

      return matchesSearch && matchesType;
    });

    return sortRows(filtered, marketSort);
  }, [marketData, marketSearch, marketTypeFilter, marketSort]);

  const marketTopStats = useMemo(() => {
    const rows = (marketData || []).filter(
      (row) => row.market_price != null && row.change_pct_1d != null
    );
    ...
    const topLoser = [...rows].sort(
      (a, b) => Number(a.change_pct_1d || 0) - Number(b.change_pct_1d || 0)
    )[0];
    ...
    <SectionShell className="mt-2">
      <div className="mb-4 flex flex-col gap-2 md:mb-5 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex items-center gap-3 text-sm text-slate-400">
          <span>{filteredAndSortedMarket.length} resultados</span>

          <button
            type="button"
            onClick={refreshAll}
            className="rounded-xl border border-slate-700/70 px-3 py-1.5 text-xs text-slate-300 transition hover:border-indigo-500 hover:text-white"
          >
            Refresh
          </button>
        </div>
      ...
      {loading && (
        <div className="mb-4 rounded-2xl border border-slate-800 bg-slate-900 px-4 py-3 text-sm text-slate-400">
