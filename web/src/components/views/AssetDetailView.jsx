...
export default function AssetDetailView({ selectedAsset, onBack, onTransactions }) {
  ...
  const chartRows = useMemo(() => {
    ...
    const source = chartMetric === "quantity" ? data?.quantity_series : data?.series;
    return (source || []).filter((row) => !start || row.date >= start);
  }, [data, range, chartMetric]);
  const quantityRows = useMemo(() => {
    const start = dateForRange(range);
    return (data?.quantity_series || []).filter((row) => !start || row.date >= start);
  }, [data, range]);
  const asset = data?.asset;
  const displayTicker = asset?.normalized_ticker || asset?.ticker || selectedAsset?.ticker;
  const first = quantityRows[0];
  ...
  return <div className="space-y-4 sm:space-y-5">
    ...
    <div className="rounded-[22px] border border-slate-800/80 bg-slate-950/70 p-4 sm:p-5">
      ...
      <div className="mt-5 h-[310px] w-full">
        <ResponsiveContainer width="100%" height="100%"><AreaChart data={chartRows} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
          ...
          <Tooltip labelFormatter={(label) => `Fecha: ${formatDate(label)}`} labelStyle={{ color: "#cbd5e1", fontWeight: 600, marginBottom: 6 }} contentStyle={{ background: "#020617", border: "1px solid #334155", borderRadius: 14 }} formatter={(v) => [chartMetric === "market_value_usd" ? formatCurrency(v, "USD") : chartMetric === "portfolio_weight_pct" ? formatPortfolioPercent(v) : formatNumber(v, 6), chartMetric === "market_value_usd" ? "Valor en cartera" : chartMetric === "portfolio_weight_pct" ? "Peso en cartera" : "Cantidad"]}/>
