...
const SOL_MODEL = process.env.DIGITAL_TWIN_EXPERIMENT_MODEL || "gpt-5.6-sol";
...
async function runSolDecisionExperiment({ messages = [], context = {}, currentProfile = {}, preflight = {}, evidenceMatrix = {} }) {
  ...
  const instructions = `Sos el Digital Investment Twin. Esta es una prueba A/B de capacidad: producí la decisión final directamente, sin web search y sin apoyarte en un auditor posterior. Usá solamente el contexto y la evidencia recibidos.
...
Para una asignación: da una conclusión clara, explica qué fue screening y qué tuvo research profundo, compara sólo con puentes soportados, propone montos concretos si la evidencia permite una decisión y explica brevemente qué evidencia cambiaría esa decisión. No inventes implementación operativa. No fabriques precisión para parecer concluyente. Español rioplatense, directo, máximo 450 palabras.`;
  ...
  if (!answer) throw new Error("Sol experiment returned an empty decision response");
  return { answer, usage: data?.usage || null, apiRequests: 1, model: data?.model || SOL_MODEL, responseId: data?.id || null };
}

module.exports = { runSolDecisionExperiment };
