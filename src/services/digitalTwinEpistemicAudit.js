...
async function auditDecision({ draft, evidenceMatrix, currentProfile, context, plan }) {
  ...
  const data = await post({
    ...
    instructions: `Auditá epistemológicamente el borrador sin investigar ni decidir de nuevo. Revisá COMO MÁXIMO 6 claims que puedan cambiar conclusión, ranking o asignación. Clasificá cada uno como fact, evidence_based_inference, confirmed_preference, assumption o unknown. Una inferencia sólo puede mover la decisión si tiene evidencia explícita y puente lógico válido. assumption/unknown no pueden moverla silenciosamente. Si A>B no está materialmente respaldado, registrá la comparación como no resuelta. Permití empates. No agregues excepciones temáticas: auditá origen y puente lógico. Si hay salto material, falsa precisión o preferencia inventada, needsRevision=true y escribí instrucciones breves.`,
...
async function reviseDecision({ draft, audit, evidenceMatrix, currentProfile, plan }) {
  ...
  const data = await post({
    model: DEFAULT_MODEL,
    reasoning: { effort: "low" },
    instructions: `Revisá el borrador sólo para corregir revisionInstructions. No investigues ni agregues hechos. assumption/unknown no pueden desempatar. Si la evidencia no separa alternativas, declaralo. Conservá conclusiones válidas y evitá falsa precisión. Español rioplatense, directo, máximo 450 palabras.`,
