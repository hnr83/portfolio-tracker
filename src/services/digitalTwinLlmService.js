...
function parseJson(text) { const cleaned=String(text||"").trim().replace(/^```json\s*/i,"").replace(/```$/i,"").trim(); return JSON.parse(cleaned); }
function isSubstantiveUserAnswer(message) { return message?.role === "user" && String(message.content || "").trim().length >= 80; }
...
async function runGuidedInterview({messages=[],context={},currentProfile=null}) {
  ...
  const {data}=await axios.post(OPENAI_RESPONSES_URL,{model:DEFAULT_MODEL,instructions:buildInstructions(forceProposal),input,max_output_tokens:1400,text:{verbosity:"low",format:{type:"json_schema",name:"investor_interview_turn",strict:true,schema:RESPONSE_SCHEMA}},store:false},{headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,"Content-Type":"application/json"},timeout:45000});
  ...
  return {...result,usage:data?.usage||null,model:data?.model||DEFAULT_MODEL,responseId:data?.id||null};
}
